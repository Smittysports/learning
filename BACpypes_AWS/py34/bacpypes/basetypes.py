...
class PortHealth(Sequence):
    sequenceElements = \
        [ Element('timestamp', DateTime, 0)
        , Element('result', ErrorType, 1)
        , Element('property', PropertyIdentifier, 2, True)
        , Element('details', CharacterString, 3, True)
        ]
...
class SCHubConnectorState(Enumerated):
    enumerations = \
        ...
        , 'connected-to-failover':2
        }
    
class SCHubConnectionState(Enumerated):
    enumerations = \
        { 'not-connected':0
        , 'connected':1
        , 'disconnected-with-errors':2
        , 'failed-to-connect':3
        }
    
class SCHubConnection(Sequence):
    sequenceElements = \
        [ Element('connection-state', SCHubConnectionState, 0)
        , Element('connect-timestamp', DateTime, 1)
        , Element('disconnect-timestamp', DateTime, 2)
        , Element('error', ErrorType, 3, True)
        , Element('error-details', CharacterString,4, True)
        ]
